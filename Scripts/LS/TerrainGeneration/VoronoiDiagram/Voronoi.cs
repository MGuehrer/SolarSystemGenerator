...
namespace csDelaunay {
	...
	public class Voronoi {
		...
		private List<SiteTriangle> triangles;
	    ...
	    private Dictionary<Vector2f, Site> SpecialPoints = new Dictionary<Vector2f, Site>();
        ...
        public void Dispose() {
			...
			foreach (SiteTriangle t in triangles) {
				t.Dispose();
			}
			triangles.Clear();
		...
		public Voronoi(List<Vector2f> points, Rectf plotBounds, int lloydIterations)
		{
		    ...
		    Init(points, plotBounds, new List<Vector2f>());
            LloydRelaxation(lloydIterations,new List<Vector2f>().ToArray());

		    CreateAndAssignSpecialPoints();
		}

        /// <summary>
        /// This creates duplicate points on the other side of the bounds
        /// This is an easy way to sow the edges of the map
        /// </summary>
	    private void CreateAndAssignSpecialPoints()
	    {
            var edgeSites = new List<Site>();
	        foreach (var site in SitesIndexedByLocation)
	        {
	            if (!site.Value.IsBorderSite) continue;
	            edgeSites.Add(site.Value);
	            foreach (var p in site.Value.OrderedColourPoints)
	            {
	                if (!site.Value.IsPointOnBorder(p.Position, PlotBounds)) continue;

	                var specialPoint = new Vector2f(p.Position.x, p.Position.y);
	                if (Math.Abs(p.Position.x - PlotBounds.x) < 0.1f)
	                {
	                    specialPoint.x = plotBounds.width + plotBounds.x;
	                }
	                else
	                {
	                    specialPoint.x = plotBounds.x;
	                }
	                if (SpecialPoints.ContainsKey(specialPoint)) continue;
	                SpecialPoints.Add(specialPoint, site.Value);
	            }
	        }

	        foreach (var p in SpecialPoints)
	        {
	            foreach (var s in edgeSites)
	            {
                    if (!s.PointsOnBorder.Any()) continue;
	                var lowPoint = s.PointsOnBorder.First();
	                var highPoint = s.PointsOnBorder.Last();
	                if (Contains(lowPoint.Position.y, highPoint.Position.y, p.Key.y))
	                {
	                    var averageWeight = (s.MapWeight + p.Value.MapWeight) / 2f;
	                    var w = (int)Math.Round(averageWeight);
                        s.OrderedColourPoints.Add(new TexturePoint(p.Key.ToPoint().x, p.Key.ToPoint().y, w));
	                }
	            }
	        }
        }
        ...
        private void Init(List<Vector2f> points, Rectf plotBounds, List<Vector2f> specialPoints)
        {
			sites = new SiteList();
            this.plotBounds = plotBounds;
            sitesIndexedByLocation = new Dictionary<Vector2f, Site>();
			AddSites(points);
            AddSites(specialPoints);
			triangles = new List<SiteTriangle>();
			edges = new List<Edge>();
		...
		private void AddSite(Vector2f p, int index) {
			...
			Site site = Site.Create(p, index, weigth, PlotBounds);
			sites.Add(site);
